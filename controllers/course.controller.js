...
import asyncHandler from "../server/middlewares/asyncHandler.middleware.js";
  ...
  if (req.file) {
    try {
      const result = await cloudinary.v2.uploader.upload(req.file.path, {
        folder: "tausif", // Save files in a folder named tausif
      });

      // If success
      if (result) {
        // Set the public_id and secure_url in array to the database in the local server...
...
export const addLectureToCourseById = asyncHandler(async (req, res, next) => {
  ...
  // All the lectures are present in the course.lectures in database...
...
export const updateCourseById = asyncHandler(async (req, res, next) => {
  ...
  const course = await Course.findByIdAndUpdate(
    id,
    {
      $set: req.body, // This will only update the fields which are being passed to update...
    },
    {
